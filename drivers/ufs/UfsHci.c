...
#define CPS             19200000		                                /* guessed on HPET_GCID.CTP = 0x031aba85 */
#define CPMS            19200
#define CPUS            19
#endif
	...
	)
{
	...
	Delay = DivU64x32(Timeout, 10, NULL) + 1;

	do {
		//
		// Access PCI MMIO space to see if the value is the tested one.
		//
		Value = read32((void *)Address) & MaskValue;
	...
	)
{
	if (UicOpcode <= UfsUicDmePeerSet) {
		...
		case 0x00:
			break;
		case 0x01:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - INVALID_MIB_ATTRIBUTE\n"));
			break;
		case 0x02:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - INVALID_MIB_ATTRIBUTE_VALUE\n"));
			break;
		case 0x03:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - READ_ONLY_MIB_ATTRIBUTE\n"));
			break;
		case 0x04:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - WRITE_ONLY_MIB_ATTRIBUTE\n"));
			break;
		case 0x05:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - BAD_INDEX\n"));
			break;
		case 0x06:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - LOCKED_MIB_ATTRIBUTE\n"));
			break;
		case 0x07:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - BAD_TEST_FEATURE_INDEX\n"));
			break;
		case 0x08:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - PEER_COMMUNICATION_FAILURE\n"));
			break;
		case 0x09:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - BUSY\n"));
			break;
		case 0x0A:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC configuration command fails - DME_FAILURE\n"));
			break;
		default:
			ASSERT(FALSE);
			break;
		...
		case 0x00:
			break;
		case 0x01:
			DEBUG_UFS((EFI_D_VERBOSE, "UIC control command fails - FAILURE\n"));
			break;
		default:
			ASSERT(FALSE);
			break;
	...
	)
{
	switch (Result) {
	case 0xF6:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Parameter Not Readable\n"));
		break;
	case 0xF7:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Parameter Not Writeable\n"));
		break;
	case 0xF8:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Parameter Already Written\n"));
		break;
	case 0xF9:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Length\n"));
		break;
	case 0xFA:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Value\n"));
		break;
	case 0xFB:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Selector\n"));
		break;
	case 0xFC:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Index\n"));
		break;
	case 0xFD:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Idn\n"));
		break;
	case 0xFE:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with Invalid Opcode\n"));
		break;
	case 0xFF:
		DEBUG_UFS((EFI_D_VERBOSE, "Query Response with General Failure\n"));
		break;
	default:
		ASSERT(FALSE);
		break;
	...
	)
{
	...
	if ((BufferSize & (BIT0 | BIT1)) != 0) {
		...
		DEBUG_UFS((EFI_D_VERBOSE, "UfsInitUtpPrdt: The BufferSize [%d] is not dword-aligned!\n", BufferSize));
	}
	...
	DEBUG_UFS((EFI_D_VERBOSE, "UfsInitUtpPrdt: Buffer addr = 0x%0x\n", (UINTN)Buffer));

	RemainingLen = BufferSize;
	Remaining    = Buffer;
	PrdtNumber   = (UINTN)DivU64x32((UINT64)BufferSize + UFS_MAX_DATA_LEN_PER_PRD - 1,
		UFS_MAX_DATA_LEN_PER_PRD, NULL);
	...
	)
{
	...
	PrdtNumber = (UINTN)DivU64x32((UINT64)Length + UFS_MAX_DATA_LEN_PER_PRD - 1,
		UFS_MAX_DATA_LEN_PER_PRD, NULL);
	...
	Trd->RuL    = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_RESPONSE_UPIU)),
		sizeof(UINT32), NULL);
	Trd->RuO    = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_COMMAND_UPIU)),
		sizeof(UINT32), NULL);
	Trd->PrdtL  = (UINT16)PrdtNumber;
	Trd->PrdtO  = (UINT16)DivU64x32((UINT64)(ROUNDUP8 (sizeof(UTP_COMMAND_UPIU)) + ROUNDUP8(sizeof(UTP_RESPONSE_UPIU))),
		sizeof(UINT32), NULL);
	return EFI_SUCCESS;
}
	...
	)
{
	...
	if (Opcode == UtpQueryFuncOpcodeWrDesc) {
		Trd->RuL  = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_QUERY_RESP_UPIU)),
			sizeof(UINT32), NULL);
		Trd->RuO  = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_QUERY_REQ_UPIU)) + ROUNDUP8(DataSize),
			sizeof(UINT32), NULL);
	} else {
		Trd->RuL  = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_QUERY_RESP_UPIU)) + ROUNDUP8(DataSize),
			sizeof(UINT32), NULL);
		Trd->RuO  = (UINT16)DivU64x32((UINT64)ROUNDUP8(sizeof(UTP_QUERY_REQ_UPIU)),
			sizeof(UINT32), NULL);
	}
	...
	)
{
	...
	Trd->RuL    = (UINT16)DivU64x32((UINT64)ROUNDUP8 (sizeof(UTP_NOP_IN_UPIU)),
		sizeof(UINT32), NULL);
	Trd->RuO    = (UINT16)DivU64x32((UINT64)ROUNDUP8 (sizeof(UTP_NOP_OUT_UPIU)),
		sizeof(UINT32), NULL);
...
UfsFindAvailableSlotInTrl(
	IN  __attribute__((__unused__))  UFS_PEIM_HC_PRIVATE_DATA     *Private,
	OUT UINT8        *Slot
	)
{
	...
	)
{
	...
	if ((Data & UFS_HC_UTRLRSR) != UFS_HC_UTRLRSR) {
		write32((void *)Address, UFS_HC_UTRLRSR);
	}

	Address = UfsHcBase + UFS_HC_UTRLDBR_OFFSET;
	write32((void *)Address, BIT0 << Slot);
}
	...
	)
{
	...
	if ((Data & (BIT0 << Slot)) != 0) {
		...
		Data    = read32((void *)Address);
		write32((void *)Address, (Data & ~(BIT0 << Slot)));
	}
}
	...
	)
{
	...
	DEBUG_UFS((EFI_D_VERBOSE, "SCSI Request Packet DataDirection = %d\n", Packet->DataDirection));
	if (Trd->Ocs == 0) {
	if (Packet->DataDirection == UfsDataIn) {
	...
	)
{
	...
	write32((void *)Address, Arg1);

	Address = UfsHcBase + UFS_HC_UCMD_ARG2_OFFSET;
	write32((void *)Address, Arg2);
	...
	)
{
	...
	//
	write32((void *)Address, UFS_HC_HCE_EN);
	//
	...
	)
{
	...
	// PAA TODO check this
	//
	status = alloc_align(&FreeBufPage, &BufPage, (EFI_SIZE_TO_PAGES(Nutmrs * sizeof(UTP_TMRD)) * EFI_PAGE_SIZE), 4096);
	if (EFI_ERROR(status)) {
		return EFI_DEVICE_ERROR;
	}
	Buffer = (UINTN)BufPage;
	Private->FreeUtpTmrlBase = FreeBufPage;
	...
	write32((void *)Address, (UINT32)(UINTN)Buffer);
	Address = Private->UfsHcBase + UFS_HC_UTMRLBAU_OFFSET;
	write32((void *)Address, (UINT32)RShiftU64((UINT64)Buffer, 32));
	...
	write32((void *)Address, UFS_HC_UTMRLRSR);
	...
	)
{
	...
	Buffer = (UINTN)BufPage;
	Private->FreeUtpTrlBase = FreeBufPage;
	...
	)
{
	...
	if (EFI_ERROR(Status)) {
		DEBUG_UFS((EFI_D_VERBOSE, "UfsDevicePei: Task management list initialization Fails, Status = %d\n", Status));
		return Status;
	}
	...
	DEBUG_UFS((EFI_D_VERBOSE, "UfsDevicePei Finished\n"));
	return EFI_SUCCESS;
}
