...
EFI_STATUS alloc_aligned_page(VOID **free_addr, VOID **aligned_addr,
						UINTN page_cnt)
{
	*free_addr = malloc((page_cnt + 1) * EFI_PAGE_SIZE);
	if (!*free_addr)
		return EFI_OUT_OF_RESOURCES;
	ZeroMem(*free_addr, (page_cnt + 1) * EFI_PAGE_SIZE);

	*aligned_addr = (VOID *)(((UINTN)*free_addr + EFI_PAGE_SIZE - 1) & ~(EFI_PAGE_SIZE - 1));
	return EFI_SUCCESS;
}

EFI_STATUS alloc_align(VOID **free_addr, VOID **aligned_addr,
					UINTN size, UINTN align)
{
	ASSERT ((align & (align - 1)) == 0);

	*free_addr = malloc(size + align);
	if (!*free_addr)
		return EFI_OUT_OF_RESOURCES;

	ZeroMem(*free_addr, size + align);
	...
	)
{
	...
	TempPtr = malloc(sizeof(UFS_PEIM_MEM_BLOCK));
	if (TempPtr == NULL) {
		return NULL;
	}
	...
	status = alloc_align(&FreeBufPage, &BufPage, Pages * EFI_PAGE_SIZE, 4096);
	if (EFI_ERROR(status)) {
		return NULL;
	}
	Address = (UINTN)BufPage;

	ZeroMem((VOID *)(UINTN)Address, EFI_PAGES_TO_SIZE (Pages));

	Block->Buf  = (UINT8 *)((UINTN)Address);
	Block->FreeBuf = FreeBufPage;
	Block->Next = NULL;
...
VOID
UfsFreeMemBlock (
	IN __attribute__((__unused__)) UFS_PEIM_MEM_POOL       *Pool,
	IN __attribute__((__unused__)) UFS_PEIM_MEM_BLOCK      *Block
	)
{
	...
	)
{
	...
	TempPtr = malloc(sizeof(UFS_PEIM_MEM_POOL));
	if (TempPtr ==  NULL) {
		return EFI_OUT_OF_RESOURCES;
	}
	...
	Pool->Head = UfsAllocMemBlock(UFS_PEIM_MEM_DEFAULT_PAGES);
	...
	)
{
	...
	//
	for (Block = Head; Block != NULL; Block = Block->Next) {
		Mem = UfsAllocMemFromBlock(Block, AllocSize / UFS_PEIM_MEM_UNIT);
	...
	NewBlock = UfsAllocMemBlock(Pages);
	if (NewBlock == NULL) {
		return NULL;
	}
	...
	//
	UfsInsertMemBlockToPool(Head, NewBlock);
	Mem = UfsAllocMemFromBlock(NewBlock, AllocSize / UFS_PEIM_MEM_UNIT);
	...
	)
{
	...
	//
	if ((Block != Head) && UfsIsMemBlockEmpty(Block)) {
		UfsFreeMemBlock(Pool, Block);
	}
