...
)
{
	...
	case 1:
		if (Read)
			*(UINT8 *)Data = read8((void *)(Address + Offset));
		else
			write8((void *)(Address + Offset), *(UINT8 *)Data);
		break;

	case 2:
		if (Read)
			*(UINT16 *)Data = read16((void *)(Address + Offset));
		else
			write16((void *)(Address + Offset), *(UINT16 *)Data);
		break;

	case 4:
		if (Read)
			*(UINT32 *)Data = read32((void *)(Address + Offset));
		else
			write32((void *)(Address + Offset), *(UINT32 *)Data);
		break;

	case 8:
		if (Read)
			*(UINT64 *)Data = *(volatile UINT64 *)(Address + Offset);
		else
...
)
{
	...
	DEBUG_NVME ((EFI_D_INFO, "ReadNvmeControllerCapabilities: Cap = 0x%llX\n", Cap));
...
)
{
	...
	DEBUG_NVME ((EFI_D_INFO, "Cc.En: %d\n", Cc->En));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Css: %d\n", Cc->Css));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Mps: %d\n", Cc->Mps));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Ams: %d\n", Cc->Ams));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Shn: %d\n", Cc->Shn));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Iosqes: %d\n", Cc->Iosqes));
	DEBUG_NVME ((EFI_D_INFO, "Cc.Iocqes: %d\n", Cc->Iocqes));
...
)
{
	...
	DEBUG_NVME ((EFI_D_INFO, "Aqa.Asqs: %d\n", Aqa->Asqs));
	DEBUG_NVME ((EFI_D_INFO, "Aqa.Acqs: %d\n", Aqa->Acqs));
...
)
{
	...
	DEBUG_NVME ((EFI_D_INFO, "NVMe controller is enabled with status [0x%x].\n", Status));
	return Status;
}
...
)
{
	...
	DEBUG_NVME ((EFI_D_INFO, "ReadNvmeControllerCapabilities: ret = 0x%X\n", Status));
	if (EFI_ERROR (Status))
		return Status;

	if (Private->Cap.Css != 0x01) {
		DEBUG_NVME ((EFI_D_INFO, "NvmeControllerInit: the controller doesn't support NVMe command set\n"));
	...
	if (Private->ControllerData == NULL) {
		Private->ControllerData = (NVME_ADMIN_CONTROLLER_DATA *)MallocZero (sizeof(NVME_ADMIN_CONTROLLER_DATA));
		if (Private->ControllerData == NULL)
	...
	DEBUG_NVME ((EFI_D_INFO, "    SQES      : 0x%x\n", Private->ControllerData->Sqes));
