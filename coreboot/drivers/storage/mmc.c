...
#define MB_TO_BLOCKS(x) (x * 1024 * 1024) / 512
...
static int __mmc_send_cmd(struct cmd *c)
{
	...
	m->host->send_cmd(m, c);
#if MMC_DEBUG
	printf("=== DEBUG CMD %d args %x ==== \n", c->index, c->args);
	printf("SDHCI_INT_ENABLE %x\n", sdhci_read16(m->host, SDHCI_INT_ENABLE));
	printf("SDHCI_TRANSFER_MODE %x\n", sdhci_read16(m->host, SDHCI_TRANSFER_MODE));
	printf("SDHCI_CMD_REG %x\n", sdhci_read16(m->host, SDHCI_CMD_REG));
	printf("SDHCI_BLOCK_SIZE %x\n", sdhci_read16(m->host, SDHCI_BLOCK_SIZE));
	printf("SDHCI_DMA_ADDR %x\n", sdhci_read32(m->host, SDHCI_DMA_ADDR));
	printf("SDHCI_HOST_CTRL %x\n", sdhci_read8(m->host, SDHCI_HOST_CTRL));
	printf("SDHCI_POWER_CONTROL %x\n", sdhci_read8(m->host, SDHCI_POWER_CONTROL));
	printf("SDHCI_CLOCK_CONTROL %x\n", sdhci_read16(m->host, SDHCI_CLOCK_CONTROL));
	printf("SDHCI_ARGUMENT %x\n", sdhci_read32(m->host, SDHCI_ARGUMENT));
	printf("SDHCI_BLOCK_CNT %x\n\n", sdhci_read16(m->host, SDHCI_BLOCK_CNT));
	printf("SDHCI_INT_STATUS %x\n", sdhci_read16(m->host, SDHCI_INT_STATUS));
#endif
...
emmc_show_hwinfo(struct mmc *m)
{
	...
	printf("BSD MMC driver: %dMB, boot %x/%x  [%s %s FW %x.%x %x %d/%d]\n",
...
static void mmc_reset(struct mmc *m)
{
	struct cmd c;
	c.index    = CMD_RESET;
	c.args     = 0x0;
...
** eMMC: cmd API
*/
static int mmc_send_cmd1(struct mmc *m)
{
	struct cmd c;
	unsigned busy = 1;
...
*/
int mmc_init_card(pcidev_t dev, bool diag)
{
	struct mmc *m = &card;
	int err;
	...
	*/
	...
	if (! host)
	{
		printf("Error SDHCI host controller not found\n");
		return 1;
	}
	...
	*/
	...
	if (err) {
		printf("Error MMC host controller not found\n");
		return 1;
	}
	...
	if (err) {
		printf(" MMC read ext csd failure\n");
		return err;
	}
	...
	if (mmc_card_hs200(m)) {
		...
		if (err) {
			printf("MMC host hs200 enabling failure\n");
			return err;
		}
		if (mmc_card_hs400(m)) {
			err = mmc_enable_hs400(m);
			if (err) {
				printf("MMC host hs400 enabling failure\n");
				return err;
			}
			printf("MMC host hs400 enabled\n");
		} else
			printf("MMC host hs200 enabled\n");
	}
	else {
		printf("MMC host hs200 not supported\n");
		return 1;
	}
